...
typedef map<int64_t, map<item, pair<int64_t, int64_t>>> gatedDataMap;

class Fpm_Gated_Connection
{
	int64_t _willExit;
	thread _thread;
	set <string> _regionSet, _ipSet;
 ...
 public:
	...
	void init (string ip, int64_t port, int64_t sleepSecond = 70)
	{
		_infosMap.init();
		_gap = 5;
		_maxDay = Setting::getInt("Cache.Max.Day.GatedConnection", 30);
		_sleepSecond = sleepSecond; 
		_lastId = _willExit = 0;
		_dbproxy = TCPClient::createClient(ip, port);
		_dbproxy -> setQuestTimeout(300);
		_thread = thread (&Fpm_Gated_Connection::cacheWork, this);
	}
	void cacheWork ()
	{
		_lastId = 0;
...
cout<<"begin slow read"<<endl;
	...
	int64_t update ()
	{
		if (_lastId == 0)
		{
			sprintf(_buf, "select id, region, ip, insert_time, num from rtm_gate_connection where insert_time > %ld order by id asc limit %d", (int64_t)slack_real_sec() - 60 * 60 * 24 * _maxDay, ONCE_QUERY_NUM);
		}
		else 
		{
			sprintf(_buf, "select id, region, ip, insert_time, num from rtm_gate_connection where id > %ld order by id asc limit %d", _lastId, ONCE_QUERY_NUM);
		}
	...
	int64_t trans (FPAnswerPtr &answer)
	{
		...
		for (auto elem: ret)
		{
			string region = elem[1], ip = elem[2], insert_time = elem[3], num = elem[4];
			setted(region, _regionSet);
			setted(ip, _ipSet);
			_infosMap.put(region, ip);
			_data[near(toDig(insert_time), _gap)][item (region, ip, "")].first += toDig(num);
			_data[near(toDig(insert_time), _gap)][item (region, ip, "")].second ++;
		}
		...
		if (split == "no")
		{
			for (auto Region: regionList)
			{
				...
				for (auto iter = _data.lower_bound(fromTime); iter != _data.end() && iter->first <= toTime; iter++)
				{
					...
					for (auto Ip: ipList)
					{
						if (iter->second.find (item (Region, Ip, "")) != iter->second.end())
							total += iter->second[item (Region, Ip, "")].first / iter->second[item (Region, Ip, "")].second;
					}
					if (total > 0)
					{
	   	...
	   	else
		{
			for (auto Ip: ipList)
			{
				...
				for (auto iter = _data.lower_bound(fromTime); iter != _data.end() && iter->first <= toTime; iter++)
				{
					...
					{
						ret.push_back(iter->first);
						ret.push_back(iter->second[item (region, Ip, "")].first / iter->second[item (region, Ip, "")].second);
					}
				}
				if (ret.size() > 0)
					tmpMap[Ip] = ret;
